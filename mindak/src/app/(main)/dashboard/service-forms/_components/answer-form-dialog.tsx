...
import {
  createServicesQuestionAnswer,
  updateServicesQuestionAnswer,
  uploadServicesAnswerImage,
...
const formSchema = z.object({
  ...
  answer_metadata: z.string().optional(),
  ...
  React.useEffect(() => {
    if (answer) {
      form.reset({
        ...
        answer_metadata: answer.answer_metadata ? JSON.stringify(answer.answer_metadata, null, 2) : "",
        order: answer.order,
    ...
    try {
      ...
      if (imageFile) {
        try {
          ...
          uploadedImageUrl = await uploadServicesAnswerImage(imageFile);
      ...
      if (values.answer_metadata) {
        try {
          answerMetadata = JSON.parse(values.answer_metadata);
        } catch {
          toast.error("Invalid JSON in answer metadata");
          return;
        }
      }

      // Merge image URL with existing metadata
      if (uploadedImageUrl) {
        answerMetadata = {
          ...answerMetadata,
      ...
      if (isEdit) {
        await updateServicesQuestionAnswer(questionId, answer.id, data);
        toast.success("Answer updated successfully");
      } else {
        await createServicesQuestionAnswer(questionId, data);
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Additional Metadata (Optional JSON)</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder='{"description": "Additional info", "color": "#0066cc"}'
                      className="font-mono text-sm"
                      {...field}
                    />
                  </FormControl>
                  <FormDescription>Additional metadata as JSON object (image will be added automatically)</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
